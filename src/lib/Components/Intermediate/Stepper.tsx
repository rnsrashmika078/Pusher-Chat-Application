  ...
  return (
    <div>
        ...
        {steps.map((step, index) => (
          <div key={index} className="z-50">
            <div className="flex flex-col gap-1 justify-center items-center">
              ...
              <p className=" text-center text-gray-600">{step.title}</p>
            </div>
          </div>
        ))}
        <div
          className={`z-0 absolute h-1 rounded-l-2xl bg-gray-600 flex justify-between mx-11 w-10/12 my-6`}
        >
          <div
      ...
      <div className="flex justify-center items-center flex-col space-y-5 my-5">
        ...
        <div>LENGTH: {steps.length}</div>
        <div>
          WIDTH CAL: {(currentStep / steps.length)}
        </div>
      </div>
    </div>
  );
